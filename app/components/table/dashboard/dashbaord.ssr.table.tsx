...
export default function DashboardSsrTableUI({ initData, session, }: { session: Session; initData: IReservation[] }) {
  const { user } = session
  const INITIAL_VISIBLE_COLUMNS = user.privilege_user === PrivilegesEnum.CLT ?
    ["#", "#n", "room", "date", "statut", "actions"] : ["#", "#n", "room", "date", "client", "statut", "actions"];
